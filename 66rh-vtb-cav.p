/*             Банковская интегрированная система БИСквит
    Copyright: (C) 1992-2002 ТОО "Банковские информационные системы"
     Filename: rh-vtb-cav.p
      Comment: Извещение о закрытии счетов 
         Uses:
      Created: Сироткин 31.05.2011
     Modified:
     На основе vtb-oav.p
...
DEFINE VARIABLE account-date   AS DATE                     NO-UNDO. /* дата закрытия счета */
...
FOR EACH tmprecid,
   ...
   DO mi=1 TO 2:
      ...
      "Сообщаем, что " + STRING(account-date,'99/99/9999') + " Вам закрыт " + account-target SKIP.
      PUT STREAM fil UNFORMATTED "счет № " + string(account,'x(20)') + " в ОАО Россельхозбанк ".
      PUT STREAM fil UNFORMATTED STRING(acct-place) + ":" SKIP (1).
      ...
      put STREAM fil unformatted "Вам необходимо в  7-и  дневный срок уведомить налоговые органы, ПФР, ФСС о" SKIP.
      put STREAM fil unformatted "закрытии вышеуказанного счета." SKIP (1).
   ...
   IF macro-prn THEN DO:
      ...
      DO i=1 TO 2:
         ...
         RUN PUT_PCL_STR(L-Sh, (i - 1) * Sec + 350 + {&OFFSET_VAL}, "Сообщаем, что " + STRING(account-date,'99/99/9999') + " Вам закрыт " + account-target, INPUT-OUTPUT buf-str).
         ...
         RUN PUT_PCL_STR(L-Sh, (i - 1) * Sec + 850 + {&OFFSET_VAL}, "Вам необходимо в  7-и  дневный срок уведомить налоговые органы, ПФР, ФСС о" , INPUT-OUTPUT buf-str).
         RUN PUT_PCL_STR(L-Sh, (i - 1) * Sec + 900 + {&OFFSET_VAL}, "закрытии вышеуказанного счета." , INPUT-OUTPUT buf-str).
