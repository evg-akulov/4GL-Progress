...
DEFINE VARIABLE NAME AS CHARACTER EXTENT 10
   FORMAT "x(200)" COLUMN-LABEL "НАИМЕНОВАНИЕ" NO-UNDO.
   ...
   DO j = 1 TO NUM-ENTRIES(dispcols):
      CASE ENTRY(j,dispcols):
         ...
         WHEN "custname" THEN ASSIGN
            ...
            header2 = header2 + " " + FILL("-",IF one-string-name THEN 100 ELSE 35)
            header3 = header3 + " " + FILL(" ",IF one-string-name THEN 100 ELSE 35)
         .
         WHEN "name" THEN ASSIGN
            header1 = header1 + " " + STRING("НАИМЕНОВАНИЕ СЧЕТА","x(" + STRING(lname) + ")")
            header2 = header2 + " " + FILL("-",IF one-string-name THEN 200 ELSE 35)
            header3 = header3 + " " + FILL(" ",IF one-string-name THEN 200 ELSE 35)
         .
         WHEN "contract" THEN ASSIGN
            header1 = header1 + " " + STRING("ПРОГРКРЕД","x(10)")
   ...
   FOR EACH ttSort USE-INDEX idxSort,
      ...
      DO j = 1 TO NUM-ENTRIES(dispcols):
         CASE ENTRY(j,dispcols):
            ...
            END.
            WHEN "t_curr" THEN do:
               PUT UNFORMATTED " " STRING(acct.branch-id,"x(12)").
...
/*               PUT UNFORMATTED " " STRING(acct.contract,"x(10)"). иркутск*/
/*               PUT UNFORMATTED " " STRING(wrkacct.cont-code, "x(35)").*/
               PUT UNFORMATTED STRING(GetXattrValueEx("loan","Кредит," + wrkacct.cont-code,"ПрогКред", ?), "x(35)").
            END.
            WHEN "val1" OR WHEN "val2" OR WHEN "val3" OR WHEN "val4" or
            WHEN "val5" OR WHEN "val6" OR WHEN "val7" OR WHEN "val8" or
            WHEN "val9" OR WHEN "val10" OR WHEN "val11" OR WHEN "val12" THEN do:
