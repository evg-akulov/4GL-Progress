/*                          
    ...
    Copyright: 01/07/2012 ИРФ 
     Filename: 66sign_rep4.p
      Comment: Отчет "Список клиентов"
   Parameters:
      Used by: 
      Created: 01/07/2012 ИРФ 
     Modified: 
*/
...
DEFINE VARIABLE DO_ 	  AS CHARACTER NO-UNDO.
def var ind1 as int64             NO-UNDO.


{setdest.i}
PUT UNFORMATTED "Клиенты " SKIP. 
PUT UNFORMATTED "Код;Наименование;Адрес;Телефон;Счета;ДО;ОснВидыДеят;Роль;ФИО;Телефон" SKIP. 
   ...
   PUT UNFORMATTED cust-corp.cust-id ";" cust-corp.name-short ";" cust-corp.addr-of-low[1] ";" GetXAttrValueEx("cust-corp",STRING(cust-corp.cust-id),"tel","-") ";" . 
	...
	FOR EACH acct WHERE  acct.cust-id = cust-corp.cust-id 
	   ...
	   acs = acs + string(acct.acct) + " ".
           DO_ = string(acct.branch-id).
	END.
	
   PUT UNFORMATTED acs ";" .
   PUT UNFORMATTED DO_ ";" .   
   PUT UNFORMATTED GetXAttrValueEx("cust-corp",STRING(cust-corp.cust-id),"ОснВидыДеят","-") ";" .   
